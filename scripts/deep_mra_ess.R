deep_mra_ess <- function(y, locs, n_mcmc = 1000,
    ...
    update_pars_alpha_x1 <- function(alpha_x1_prop, pars) {
        ...
        pars$W2 <- eval_basis(cbind(pars$W1 %*% pars$alpha_x1, pars$W1 %*% pars$alpha_y1), pars$grid, M=pars$M, n_coarse_grid = pars$n_coarse_grid)$W
        pars$W  <- eval_basis(cbind(pars$W2 %*% pars$alpha_x2, pars$W2 %*% pars$alpha_y2), pars$grid, M=pars$M, n_coarse_grid = pars$n_coarse_grid)$W
        return(pars)
    }
    ...
    W1 <- eval_basis(as.matrix(locs), grid, M=M, n_coarse_grid = n_coarse_grid)$W
    ...
    W2 <- eval_basis(cbind(W1 %*% alpha_x1, W1 %*% alpha_y1), grid, M=M, n_coarse_grid = n_coarse_grid)$W
    ...
    W <- eval_basis(cbind(W2 %*% alpha_x2, W2 %*% alpha_y2), grid, M=M, n_coarse_grid = n_coarse_grid)$W
